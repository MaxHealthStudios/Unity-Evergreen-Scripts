...
/* HOW TO USE:
 - All collision environments must be vertical or horzizontal, nothing diagonal.
 - All collision environments must have the layer "Environment".
 - One-way platforms must include the tag "one way".
...
public class PlatformerController2DFlat : MonoBehaviour {
    ...
    private float minY, maxY, minX, maxX;//Used to constrain the player position
    ...
    void RaycastDown(int rayCount) {
        ...
        for(int i = 0; i < rayCount; i++) {
            ...
            if(hit == true && hit.transform.tag == "one way" && velocity.y <= 0) {
    ...
    void RaycastUp(int rayCount) {
        ...
        for(int i = 0; i < rayCount; i++) {
            RaycastHit2D hit = Physics2D.Raycast(rayOrigin + new Vector2(raySpacing * i, 0), Vector2.up, rayLength, LayerMask.GetMask("Environment"));
